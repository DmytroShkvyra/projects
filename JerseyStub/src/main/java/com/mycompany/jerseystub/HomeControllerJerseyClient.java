 ...
 * @author User
 */
public class HomeControllerJerseyClient {
    private WebResource webResource;
    private Client client;
    private static final String BASE_URI = "http://localhost:8084/GuiceTest/";

    public HomeControllerJerseyClient() {
        com.sun.jersey.api.client.config.ClientConfig config = new com.sun.jersey.api.client.config.DefaultClientConfig();
        //config.getClasses().add(PlanetJAXBContextProvider.class);
...
Client client = Client.create(config);*/
    ...
    public <T> T throwErrorHTML(Class<T> responseType) {
        WebResource resource = webResource;
        resource = resource.path("throw");
        T res = null;
        try{
        res = resource.get(responseType);
